...
bot.on('message', (msg) => {
  ...
  console.log(msg_text);	
      ...
      bot.getChatMember(msg.chat.id, msg.from.id).then(function(data) {
          console.log(timeStamp() + " Member check:");
          console.log(data);
          if ((data.status == "creator") || (data.status == "member") || (data.status == "administrator")){
              ...
              if (err) throw err;
              console.log(timeStamp() + " Message Insert oki: " + result.rowsAffected);
              sql.close();
              }); // or requ
          });     
          }else{
...
bot.on('photo',(msg) => {
    ...
    //end get file link
	if(grChck.indexOf(msg.from.id) > 0 ) {
		console.log(timeStamp() + " Check ID in group,ok");
		console.log(timeStamp(),grChck.indexOf(msg.from.id));
		...
		fileStream.on('info', (info) => {
			...
			console.log(timeStamp(),logFileName);
			fs.appendFile(logFileName,text, function (err) {
				if (err) throw err;
				console.log(timeStamp() +' Log Saved!');
				});
            async function download (){
                console.log(timeStamp() + " Download photo:",fileId); //downloadFile vào ổ c:
   ...
   if(grChck.indexOf(msg.from.id) > 0 ) {
    ...
    fileStream.on('info', (info) => {
     //console.log(info);
      ...
      console.log(timeStamp(), UriFile); 
            //
            async function download (){
                console.log(timeStamp() +" Download file:",fileName); //downloadFile vào ổ c:
                    ...
                    try {
                        ...
                        console.log(timeStamp() +" Successfully renamed the file!",{fileName,docName})
                      } catch(err) {
                        throw err
                      }
                    //await Insert();
...
console.log(timeStamp() + "End logggg =================================")  