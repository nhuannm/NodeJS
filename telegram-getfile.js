     ...
     async () => {
        try {
            ...
            let result1 = await pool.request();
                ...
                result1.input('msg_id',msg_id);
                result1.input('msg_date',msg_date);
                result1.input('From_Id',From_Id);
                result1.input('From_name',From_name);
                result1.input('chat_id',chat_id);
                result1.input('chat_name',chat_name);
                result1.input('chat_title',chat_title);
                result1.input('chat_type',chat_type);
                result1.input('msg_text',msg_text);
                result1.input('doc_filename',fileName);
                result1.input('info_uri',UriFile);
                
                var sqlInsert = "INSERT INTO Telegram_log(msg_id,Msg_date,From_Id,From_name,chat_id,chat_name,chat_title,chat_type,msg_text,doc_filename,info_uri) VALUES(@msg_id,@Msg_date,@From_Id,@From_name,@chat_id,@chat_name,@chat_title,@chat_type,@msg_text,@doc_filename,@info_uri) "
                result1.query(sqlInsert);//,function (err, result) {
...
console.log('msg_text: ' + msg_text);
   ...
   if(grChck.indexOf(msg.from.id) > 0 ) {
    ...
    console.log(bot.downloadFile(docId,"c:/Telegramsms/")); //downloadFile vào ổ c:
